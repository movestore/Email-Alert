      ...
      logger.info(paste("Your alert condition:",variab," ",rel," [",valu,"] will be checked."))
        ...
        if (rel=="%in%") #works for numeric or character values
        {
          ...
          if (any(data@data[,variab] %in% valus))
          {
            ...
            if (length(selix)>1) writeLines(c(emailtext,paste("Your following Alert Condition is fullfilled:",variab,rel,valu,"(for", nloc, "locations of", nani, "animals)."),"See your max. 10 unique (maximum value) data rows (if attr specified):",attr,as.vector(selixo10)), paste0(Sys.getenv(x = "APP_ARTIFACTS_DIR", "/tmp/"),"email_alert_text.txt"))
          } else 
          {
