      ...
      if (variab %in% names(data@data))
      {
        ...
        if (rel=="%in%") #works for numeric or character values
        {
          ...
          if (any(data@data[,variab] %in% valus))
          {
            ...
            if (nloc>=minlocs & nani>=minanimals) writeLines(c(emailtext,paste("Your following Alert Condition is fullfilled:",variab,rel,valu,"for", nloc, ">=",minlocs,"locations of", nani, ">=", minanimals,"animals.")), paste0(Sys.getenv(x = "APP_ARTIFACTS_DIR", "/tmp/"),"email_alert_text.txt"))
          } else 
          {
